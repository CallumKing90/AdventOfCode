...
const data2 = `LLLL.LLLLL.LLLLLLLLLLLLLLLL.L.LLLLLL.LLLL..L.LLLLLLLL.LLLLLL.LLLL..LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL
...
let tidyData = data.split(/\n/g);
let occupiedSeats = 0;

tidyData.forEach((row, index) => {
  tidyData[index] = row.split("");
});

function makeSeatingChange(callback) {
  let currentlyOccupiedSeats = 0;
  const newData = [];

  tidyData.forEach((row, rowIndex) => {
    newData.push([]);

    row.forEach((rowItem, rowItemIndex) => {
      const result = checkIfAdjacentSeatsOccupied(rowIndex, rowItemIndex);
  ...
  if (currentlyOccupiedSeats !== occupiedSeats) {
    tidyData = newData;
    occupiedSeats = currentlyOccupiedSeats;
    callback(callback);
  } else {
    console.log(
      `currentlyOccupiedSeats:`,
...
function checkIfAdjacentSeatsOccupied(row, column) {
  let occupiedSeats = 0;

  let rowAbove = row > 0 ? row - 1 : false;
  let rowBelow = tidyData.length - 1 > row ? row + 1 : false;
  let itemLeft = column > 0 ? column - 1 : false;
  let itemRight = tidyData[row].length - 1 > column ? column + 1 : false;

  if (itemLeft) {
    if (tidyData[row][itemLeft] === "#") occupiedSeats++;
  }

  if (itemRight) {
    if (tidyData[row][itemRight] === "#") occupiedSeats++;
  }

  if (rowAbove) {
    if (tidyData[rowAbove][column] === "#") occupiedSeats++;

    if (itemLeft) {
      if (tidyData[rowAbove][itemLeft] === "#") occupiedSeats++;
    }

    if (itemRight) {
      if (tidyData[rowAbove][itemRight] === "#") occupiedSeats++;
    }
  }

  if (rowBelow) {
    if (tidyData[rowBelow][column] === "#") occupiedSeats++;

    if (itemLeft) {
      if (tidyData[rowBelow][itemLeft] === "#") occupiedSeats++;
    }

    if (itemRight) {
      if (tidyData[rowBelow][itemRight] === "#") occupiedSeats++;
    }
  }
...
makeSeatingChange(makeSeatingChange);
