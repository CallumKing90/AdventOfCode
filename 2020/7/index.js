...
const data2 = `dotted blue bags contain 3 wavy bronze bags, 5 clear tomato bags.
...
const shinyGold = "shiny gold bag";
let canHoldShinyGold = [];
let shinyGoldContains = [];
let totalBags = 1;

let bagQuantity;

// Part 1
function findBagsThatContainShinyGold() {
  let newBagAdded = false;

  splitArray.forEach((rule) => {
    const ruleParts = rule.split(/contain/g);

    if (ruleParts[1].match(new RegExp(shinyGold, "g"))) {
      const matchedBag = ruleParts[0].trim();
      const singularMatchedBag = matchedBag.replace(/s$/g, "");

      if (canHoldShinyGold.indexOf(singularMatchedBag) === -1) {
        newBagAdded = true;
        canHoldShinyGold.push(singularMatchedBag);
      }
    }

    canHoldShinyGold.forEach((canHold) => {
      if (ruleParts[1].match(new RegExp(canHold, "g"))) {
        const matchedBag = ruleParts[0].trim();
        const singularMatchedBag = matchedBag.replace(/s$/g, "");
  ...
  if (newBagAdded) findBagsThatContainShinyGold();
}

// Part 2
function findBagsThatBagContains({ bagName = "shiny gold bag", quantity = 1 }) {
  console.log(`### NEW CALL ###`);
  bagQuantity = quantity;
  console.log(`bag: ${bagName}, quantity: ${bagQuantity}`);

  for (var j = 0; j < splitArray.length; j++) {
    const rule = splitArray[j];
    const ruleParts = rule.split(/contain/g);
    const matchedBag = ruleParts[0].trim();
    const singularMatchedBag = matchedBag.replace(/s.$/g, "");

    let thisItem = bagName;

    if (singularMatchedBag.match(new RegExp(thisItem, "g"))) {
      console.log(`${bagQuantity} ${thisItem} contains:`);

      const insideBag = ruleParts[1].split(",");

      for (var i = 0; i < insideBag.length; i++) {
        const bag = insideBag[i];

        const matchedBag = bag.trim();
        const singularMatchedBag = matchedBag.replace(/s.$/g, "");
        if (bag.match(new RegExp("no other bag", "g"))) {
          console.log(`no other bags`);
          console.log("---");

          return;
        }
        console.log(singularMatchedBag);
        const number = singularMatchedBag.substr(0, 1);
        const wholeBag = singularMatchedBag.substr(2);
        console.log(`${number} * ${bagQuantity} = ${number * bagQuantity}`);
        totalBags += number * bagQuantity;
        console.log(`totalBags: ${totalBags}`);
        console.log("---");

        findBagsThatBagContains({ bagName: wholeBag, quantity: number });
      }
    }
  }
}

findBagsThatBagContains({ bagname: shinyGold, quantity: 1 });
